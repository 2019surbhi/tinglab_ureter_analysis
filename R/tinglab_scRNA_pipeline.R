...
#saveRDS(obj.list, paste0(args$output_dir,args$file_prefix,'obj_list.rds'))
...
for(j in 1:length(res))
   ...
   obj_sil<-iterative_clus_by_res(obj.integrated,res=res[j],dims_use=1:pc[i],verbose=args$verbose)
    get_silhouette_plot(obj_sil,reduction='pca',dims=1:pc[i],out_dir=args$output_dir,file_prefix=sil_run,verbose=args$verbose)
   }
}
